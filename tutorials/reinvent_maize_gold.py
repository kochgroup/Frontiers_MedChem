...
# 1. Hardcode the exact, absolute path to your project root to guarantee no mix-ups
project_root = "/nfs/home/myuecel3/Frontiers_MedChem"

# 2. Force it to the very front of Python's brain
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# 3. Print the path so we can visually confirm it worked!
print(f"DEBUG: Python's #1 search directory is now: {sys.path[0]}")
print(f"DEBUG: Does the 'nodes' folder exist here? {os.path.exists(os.path.join(project_root, 'nodes'))}")

# 4. NOW try the import using the classes we defined in __init__.py
